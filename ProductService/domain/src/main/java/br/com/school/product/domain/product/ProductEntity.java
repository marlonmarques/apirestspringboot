...
public class ProductEntity {
    ...
    private void selfValidate() {
        ...
        final var nameLengh = name.trim().length();

        if (nameLengh < 5 || nameLengh > 200) {
            notification.append(new Error("name must to between 5 and 200 characters"));
        }
        ...
        if (price.compareTo(cost) <= 0) {
            notification.append(new Error("price must be greeter than cost"));
        }
